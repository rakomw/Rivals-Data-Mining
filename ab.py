"""
AdaBoost classifier for Marvel Rivals win prediction.
Includes training, evaluation, SHAP analysis, and hyperparameter tuning.
AdaBoost uses adaptive boosting - focusing on misclassified samples.
"""
import pandas as pd
import numpy as np
...
def evaluate(model: AdaBoostClassifier,
             X_test: pd.DataFrame,
...
def get_shap_values(model: AdaBoostClassifier,
                    X_test: pd.DataFrame,
    ...
    Args:
        model: Trained model.
        X_test: Test features.
        max_samples: Maximum samples for SHAP calculation (smaller for AdaBoost).
    ...
    # Use KernelExplainer for AdaBoost (TreeExplainer doesn't work well with AdaBoost)
    explainer = shap.KernelExplainer(model.predict_proba, shap.sample(X_sample, 100))
    shap_values = explainer.shap_values(X_sample.iloc[:100])  # Limit for speed

    return shap_values, explainer, X_sample.iloc[:100]


def plot_shap_summary(shap_values, X_sample: pd.DataFrame,
                      output_path: str = 'data/model_results/adaboost_shap_summary.png'):
...
def plot_feature_importance(model: AdaBoostClassifier,
                            feature_names: list,
                            output_path: str = 'data/model_results/adaboost_feature_importance.png',
                            top_n: int = 20):
    ...
    plt.title('AdaBoost Feature Importance (Top 20)')
...
def plot_confusion_matrix(cm: list,
                          output_path: str = 'data/model_results/adaboost_confusion_matrix.png'):
...
def plot_roc_curve(model: AdaBoostClassifier,
                   X_test: pd.DataFrame,
                   y_test: pd.Series,
                   output_path: str = 'data/model_results/adaboost_roc_curve.png'):
    """Plot ROC curve.
...
def cross_validate(X: pd.DataFrame, y: pd.Series,
    ...
    model = AdaBoostClassifier(estimator=base_estimator, random_state=42, **model_params)
...
def main():
    """Run full AdaBoost pipeline."""
    print("="*60)
    print("ADABOOST - Marvel Rivals Win Prediction")
    print("="*60)
    ...
    # SHAP analysis (limited due to computational cost)
    print("\nGenerating SHAP values (this may take a while for AdaBoost)...")
    try:
    ...
    with open('data/model_results/adaboost_results.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print("\nResults saved to data/model_results/adaboost_results.json")
