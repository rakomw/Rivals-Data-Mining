...
\begin{itemize}
	...
	\item 	Randomized flipping: With 50\% probability, I flipped the entire match—team_one $\Leftrightarrow$ team_two, scores, and winner. This prevents the model from learning position-based shortcuts.
...
Several extensions could improve both the efficiency and usability of this project.

\textbf{Training efficiency improvements.} While the current NumPy implementation trains successfully on a large dataset, training speed could be improved by introducing more optimized optimization techniques and infrastructure. Future versions could incorporate adaptive optimizers (e.g., Adam), momentum-based updates, mixed precision or GPU acceleration (e.g., using PyTorch), and faster data loading via memory-mapped arrays or chunked training. In addition, feature selection or dimensionality reduction could reduce input size while preserving predictive signal, improving convergence speed and runtime.

\textbf{Improving the hero recommendation system.} The current recommender evaluates candidates using typical per-hero statistics derived from the dataset. Future work could improve recommendation quality by incorporating stronger composition constraints (e.g., dynamically enforcing role balance), modeling synergy beyond a static team-up list (e.g., learned synergy weights from co-occurrence statistics), and adding opponent-aware counters. Another extension would be confidence calibration, so probability outputs better reflect real-world uncertainty and produce more consistent recommendation confidence across different input scenarios.

\textbf{Web-based implementation for real users.} To make the system usable outside the command line, the project could be deployed as a web application. A lightweight backend API could load the trained model weights, accept hero inputs from users, and return win probability and role-based recommendations. A simple frontend could allow players to select heroes through a searchable interface, optionally include map selection, and visualize predicted probabilities and recommended picks. This would transform the project from a research prototype into an interactive tool that players and analysts could use in real drafting scenarios. (KD)
