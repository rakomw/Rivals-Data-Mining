...
\begin{abstract}
This project investigates predicting match outcomes in Marvel Rivals using machine learning. It focuses on transforming raw match logs into a structured dataset, engineering team-level and player-level features, and training a neural network to predict match winners. Using over 310{,}000 cleaned matches, the model captures nonlinear relationships such as role composition, hero synergy, and performance consistency, achieving over 91\% accuracy across training, validation, and test sets. Part of the motivation for this project was my experience as an e-sport's player. Being able to make decisions off of simple statistics from a game is huge for the industry and a growing need in a data driven society.
\end{abstract}
...
Once cleaned, the dataset allowed me to visualize behavior across roles and match dynamics—from average Tank role minutes to per-minute efficiency distributions. These insights shaped the feature engineering for the neural network and guided the eventual hero recommendation system. 
