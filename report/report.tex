...
We implemented a RAG pipeline to enable natural language exploration of match data. The system converts match records into text documents, embeds them using \texttt{mxbai-embed-large}, and stores vectors in ChromaDB with HNSW indexing (L2 distance). Document retrieval uses cosine similarity:
\[
...
\textbf{Winner Prediction:} The RAG system can also predict match winners by analyzing team statistics and comparing against learned patterns, achieving 80\% accuracy on test samples (16/20 correct).
...
\begin{table}[H]
    \centering
    \caption{Tree-Based Model Comparison (330,914 matches)}
    ...
    \begin{tabular}{lccc}
        ...
        XGBoost & \textbf{89.97\%} & \textbf{97.16\%} & \textbf{89.81\%} \\
